...
- **Still TODO:** (M4) interactive proof *construction* — applying a tactic/suggestion currently edits the tree and re-runs the round-trip for goals, but the suggestion *computation* (rewrite/apply/induction suggestions, term-builder) still calls the TT engine; needs Lean-backed suggestion sources. MathRow→Lean-source round-trip for type edits. Validation/error display polish.
