...
- Converged hypothesis/projection actions onto the same shared suggestion pipeline as goal-body tactics: selected-hypothesis exact/apply/destructure/projection clicks now go through the generic `applySuggestionToProofTreeState` path instead of a dedicated hypothesis-only action helper, projection suggestions carry source-aligned `have` tactic commands directly, and induction-on-hypothesis suggestions reuse the same induction path as the main tactic view. Deleted the dead `havePrefill` UI plumbing from `ProofTreeEditor`, moved projection application source synthesis into the shared tactic-command bridge, and added bridge/editing regressions for the new shared path. Full verification green (`npx tsc --noEmit`, `npm test`: 200 files, 3145 passed, 4 skipped).
