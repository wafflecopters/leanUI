...
- **M3 — IN PROGRESS (WYSIWYG math on Lean):** The math editor renders REAL Lean expressions. `lean/Extract.lean` now emits `typeTagged`/`valueTagged` per declaration via `ppExprTagged` (Lean's `CodeWithInfos` = tagged pretty-print tree with subexpr positions). New pure converter `src/lean/codeWithInfos.ts` (`codeWithInfosToMathRow`) turns that tagged tree into the existing `MathRow` model — wrapping each subexpression in a `Group{htmlId}` (the click-to-rewrite target), tokenizing text runs into symbols, mapping Lean unicode (→, ≤, ℕ…) to LaTeX. The whole math editor (render/edit/navigate/round-trip) is reused unchanged; only the PRODUCER is new. `LeanMathView` renders it via the editor's `renderStaticLatex` + KaTeX; the `/` editor's Declarations panel now shows WYSIWYG math (type + value) with text fallback. Verified end-to-end through the bridge (subexpr paths `/0`,`/1`,`/1/0/1` intact). Tests: `src/lean/codeWithInfos.test.ts` (11). **Still TODO in M3:** interactive `MathEditor` (not just static view); tagged GOALS (goals are still plain `ppGoal` strings — need `ppExprTagged` on goal target/hyps); re-add full `WYSIWYGPanel` + structured proof tree.
