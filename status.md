...
- Keep cashing out convergence as deletions: migrate the remaining inline `have`/hoist/term-builder flows in `ProofTreeEditor` off bespoke tree surgery and into shared tactic-edit helpers or source-aligned command transforms
- Keep cashing out convergence as deletions: push the same shared tactic-edit layer into the remaining term-builder slot update flow and any proof-tree-only convenience actions that still bypass the text-tactic/command bridge
