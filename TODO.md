...
6. IN PROGRESS — presentation battery DONE (limitAdd + triangleSum, 8/8). Vector-space preset LANDED (Field'/VectorSpace/combo/InSpan/Spans/Independent/Basis in core Prop; nilIndependent + nilSpanCombo proved; spanDrop and basisExists are the sorry exercises). NEXT: prove spanDrop + basisExists (strong induction on list length, classical case on independence), seed the proof, add its battery describe.
