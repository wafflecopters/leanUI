...
function isConstructorPossible(
  ...
  for (let i = 0; i < ctorIndices.length; i++) {
    ...
    // Use flexibleVars: true because the constructor and expected type are in different contexts.
    // We treat variables as flexible (unifiable) rather than rigid skolems.
    // This ensures that the same variable appearing in different positions (like `a` twice in `Equal A a a`)
    // can be properly matched.
    const result = unifyTerms(ctorIdx, expIdx, { flexibleVars: true });
    if (!result.success) {
      // Even with flexible vars, some conflicts are real impossibilities
      // (e.g., Const vs Const with different names)
      return false;
    }
  }
