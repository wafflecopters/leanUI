...
describe('applyImplAnnotationsForBlock', () => {
  ...
  test('registers int implementations after nat impls are available', () => {
    const definitions = compileTTFromText(natIntSource).definitions;
    applyImplAnnotationsForBlock(
      {
        blockIndex: 0,
        sourceLines: [],
    ...
    applyImplAnnotationsForBlock(
      {
        ...
        nameResolutionErrors: [],
        declarations: [
          { name: 'Int', kind: 'inductive', checkSuccess: true, checkErrors: [], syntax: '@impl=int' } as any,
        ],
        isComment: false,
      },
      definitions,
    ...
    expect(definitions.intImplByCtor?.get('OfNat')?.inductiveName).toBe('Int');
    expect(definitions.intImplByCtor?.get('NegSucc')?.inductiveName).toBe('Int');
    expect(definitions.intImplByCtor?.get('OfNat')?.natImplName).toBe('Nat');
  });
});
