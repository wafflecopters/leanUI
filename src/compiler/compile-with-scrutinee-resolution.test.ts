...
describe('compile-with-scrutinee-resolution', () => {
  ...
  test('substituteHoles leaves rational literals untouched', () => {
    const ratLiteral = { tag: 'RatLit', num: 7n, den: 9n } as any;
    expect(substituteHoles(ratLiteral, new Map())).toBe(ratLiteral);
  });
});
