...
addLeBoth R a b c d hab hcd = (CompleteOrderedField.leTrans (field R) (radd a c) (radd b c) (radd b d) (addLeRight R a b c hab) (CompleteOrderedField.addLeLeft (field R) c d b hcd))
...
oneLeTwo R = (replace (\\z => rle z (rtwo R)) (addZeroLeft R (rone R)) (addLeRight R (rzero R) (rone R) (rone R) (CompleteOrderedField.zeroLeOne (field R))))
...
halfMulEpsNe R e hle hne heq = hne (trans (sym (addZeroLeft R (rzero R))) (trans (cong (\\z => radd z z) heq) (halfMulEps R e)))
...
addSumNeg : (R : Real) -> (a b : Carrier R) -> Equal (radd (radd a b) (rneg a)) b := by
  intros R a b
  erw (CompleteOrderedField.addComm (field R) (radd a b) (rneg a)), (sym (CompleteOrderedField.addAssoc (field R) (rneg a) a b)), (negLeft R a), (addZeroLeft R b)

negAddCancel : (R : Real) -> (a b : Carrier R) -> Equal (radd (radd a b) (radd (rneg a) (rneg b))) (rzero R) := by
  intros R a b
  erw (sym (CompleteOrderedField.addAssoc (field R) (radd a b) (rneg a) (rneg b))), (addSumNeg R a b), (CompleteOrderedField.negRight (field R) b)

negUnique : (R : Real) -> (a b : Carrier R) -> Equal (radd a b) (rzero R) -> Equal b (rneg a) := by
  intros R a b h
  erw (sym (addZeroLeft R b)), (sym (negLeft R a)), (CompleteOrderedField.addAssoc (field R) (rneg a) a b), h, (CompleteOrderedField.addZeroRight (field R) (rneg a))

negAdd : (R : Real) -> (a b : Carrier R) -> Equal (rneg (radd a b)) (radd (rneg a) (rneg b)) := by
  intros R a b
  erw (sym (negUnique R (radd a b) (radd (rneg a) (rneg b)) (negAddCancel R a b)))
...
subAddSub : (R : Real) -> (a b c d : Carrier R) -> Equal (rsub (radd a b) (radd c d)) (radd (rsub a c) (rsub b d)) := by
  intros R a b c d
  erw (cong (\\z => radd (radd a b) z) (negAdd R c d)), (fourTermRearrange R a b (rneg c) (rneg d))
...
coreEstimate R f g x0 L M he x hfx hgx = (leLtTrans R (rabs (rsub (radd (f x) (g x)) (radd L M))) (radd (rabs (rsub (f x) L)) (rabs (rsub (g x) M))) (radd he he) (replace (\\z => rle (rabs z) (radd (rabs (rsub (f x) L)) (rabs (rsub (g x) M)))) (sym (subAddSub R (f x) (g x) L M)) (CompleteOrderedField.absTriangle (field R) (rsub (f x) L) (rsub (g x) M))) (addLtBoth R (rabs (rsub (f x) L)) he (rabs (rsub (g x) M)) he hfx hgx))
...
convertEps R eps v hlt = replace (\\z => rlt v z) (halfMulEps R eps) hlt
...
limitAdd : (R : Real) -> (f g : Carrier R -> Carrier R) -> (x0 L M : Carrier R) -> Limit R f x0 L -> Limit R g x0 M -> Limit R (\\x => radd (f x) (g x)) x0 (radd L M)
limitAdd R f g x0 L M limF limG := by
  constructor
  intros eps heps
  have dF := Limit.eps_delta limF (rmul (rhalf R) eps) (halfMulEpsPos R eps heps)
  have dG := Limit.eps_delta limG (rmul (rhalf R) eps) (halfMulEpsPos R eps heps)
  cases (CompleteOrderedField.leTotal (field R) (DPair.fst dF) (DPair.fst dG)) with
  | Left hle =>
    exact (MkDPair (DPair.fst dF) (MkPair (Pair.fst (DPair.snd dF)) (\\x hx0 hxd => convertEps R eps (rabs (rsub (radd (f x) (g x)) (radd L M))) (coreEstimate R f g x0 L M (rmul (rhalf R) eps) x (Pair.snd (DPair.snd dF) x hx0 hxd) (Pair.snd (DPair.snd dG) x hx0 (ltLeTrans R (rabs (rsub x x0)) (DPair.fst dF) (DPair.fst dG) hxd hle))))))
  | Right hle =>
    exact (MkDPair (DPair.fst dG) (MkPair (Pair.fst (DPair.snd dG)) (\\x hx0 hxd => convertEps R eps (rabs (rsub (radd (f x) (g x)) (radd L M))) (coreEstimate R f g x0 L M (rmul (rhalf R) eps) x (Pair.snd (DPair.snd dF) x hx0 (ltLeTrans R (rabs (rsub x x0)) (DPair.fst dG) (DPair.fst dF) hxd hle)) (Pair.snd (DPair.snd dG) x hx0 hxd)))))
...
diffQuotAddEq : (R : Real) -> (f g : Carrier R -> Carrier R) -> (x0 x : Carrier R) -> Equal (radd (diffQuot R f x0 x) (diffQuot R g x0 x)) (diffQuot R (\\y => radd (f y) (g y)) x0 x) := by
  intros R f g x0 x
  erw (sym (CompleteOrderedField.distribRight (field R) (rsub (f x) (f x0)) (rsub (g x) (g x0)) (rinv (rsub x x0)))), (cong (\\z => rmul z (rinv (rsub x x0))) (sym (subAddSub R (f x) (g x) (f x0) (g x0))))

-- THE DERIVATIVE THEOREM: (f+g)' = f' + g'
derivAdd : (R : Real) -> (f g : Carrier R -> Carrier R) -> (x0 L M : Carrier R) -> HasDerivative R f x0 L -> HasDerivative R g x0 M -> HasDerivative R (\\x => radd (f x) (g x)) x0 (radd L M)
derivAdd R f g x0 L M hf hg = limitExt R (\\x => radd (diffQuot R f x0 x) (diffQuot R g x0 x)) (diffQuot R (\\y => radd (f y) (g y)) x0) x0 (radd L M) (diffQuotAddEq R f g x0) (limitAdd R (diffQuot R f x0) (diffQuot R g x0) x0 L M hf hg)
...
mulZeroLeft R a = addCancelRight R (rmul (rzero R) a) (rzero R) (rmul (rzero R) a) (trans (sym (CompleteOrderedField.distribRight (field R) (rzero R) (rzero R) a)) (trans (cong (\\z => rmul z a) (addZeroLeft R (rzero R))) (sym (addZeroLeft R (rmul (rzero R) a)))))
...
mulNegRight R c b = negUnique R (rmul c b) (rmul c (rneg b)) (trans (sym (CompleteOrderedField.distribLeft (field R) c b (rneg b))) (trans (cong (\\z => rmul c z) (CompleteOrderedField.negRight (field R) b)) (mulZeroRight R c)))

-- Helper: c*(a-b) = c*a - c*b
mulSubDistrib : (R : Real) -> (c a b : Carrier R) -> Equal (rmul c (rsub a b)) (rsub (rmul c a) (rmul c b)) := by
  intros R c a b
  erw (CompleteOrderedField.distribLeft (field R) c a (rneg b)), (mulNegRight R c b)
...
diffQuotScalarEq : (R : Real) -> (c : Carrier R) -> (f : Carrier R -> Carrier R) -> (x0 x : Carrier R) -> Equal (rmul c (diffQuot R f x0 x)) (diffQuot R (\\y => rmul c (f y)) x0 x) := by
  intros R c f x0 x
  erw (sym (CompleteOrderedField.mulAssoc (field R) c (rsub (f x) (f x0)) (rinv (rsub x x0)))), (cong (\\z => rmul z (rinv (rsub x x0))) (mulSubDistrib R c (f x) (f x0)))
...
subCancel : (R : Real) -> (a b : Carrier R) -> Equal (radd (rsub a b) b) a := by
  intros R a b
  erw (CompleteOrderedField.addAssoc (field R) a (rneg b) b), (negLeft R b), (CompleteOrderedField.addZeroRight (field R) a)

-- |c| > 0 when c /= 0
absPos : (R : Real) -> (c : Carrier R) -> (Equal c (rzero R) -> Void) -> rlt (rzero R) (rabs c)
absPos R c hne := by
  constructor
...
-- a <= b => 0 <= b - a
leToSubNonneg : (R : Real) -> (a b : Carrier R) -> rle a b -> rle (rzero R) (rsub b a)
leToSubNonneg R a b hab := by
  rewrite (sym (subSelf R a))
  exact (addLeRight R a b (rneg a) hab)
...
mulLeLeft R c a b hc hab = replace (\\z => rle (rmul c a) z) (subCancel R (rmul c b) (rmul c a)) (replace (\\z => rle (rmul c a) (radd z (rmul c a))) (mulSubDistrib R c b a) (replace (\\z => rle z (radd (rmul c (rsub b a)) (rmul c a))) (addZeroLeft R (rmul c a)) (addLeRight R (rzero R) (rmul c (rsub b a)) (rmul c a) (CompleteOrderedField.mulNonneg (field R) c (rsub b a) hc (leToSubNonneg R a b hab)))))
...
oneLeAbsPlusOne R c = replace (\\z => rle (rone R) z) (CompleteOrderedField.addComm (field R) (rone R) (rabs c)) (replace (\\z => rle z (radd (rone R) (rabs c))) (CompleteOrderedField.addZeroRight (field R) (rone R)) (CompleteOrderedField.addLeLeft (field R) (rzero R) (rabs c) (rone R) (CompleteOrderedField.absNonneg (field R) c)))

-- |c| + 1 > 0
absPlusOnePos : (R : Real) -> (c : Carrier R) -> rlt (rzero R) (radd (rabs c) (rone R))
absPlusOnePos R c = ltLeTrans R (rzero R) (rone R) (radd (rabs c) (rone R)) (zeroLtOne R) (oneLeAbsPlusOne R c)
...
mulLeRight R a b c hab hc = replace (\\z => rle z (rmul b c)) (CompleteOrderedField.mulComm (field R) c a) (replace (\\z => rle (rmul c a) z) (CompleteOrderedField.mulComm (field R) c b) (mulLeLeft R c a b hc hab))
...
epsOverMPos R eps M heps hM := by
  ...
  · exact (\\heq => Pair.snd heps (trans heq (trans (cong (\\z => rmul z (rinv M)) (sym (mulZeroLeft R (rinv M)))) (sym (CompleteOrderedField.mulAssoc (field R) (rzero R) (rinv M) (rinv M))))))
...
scalarAbsBound R c a b = replace (\\z => rle (rabs z) (rmul (radd (rabs c) (rone R)) (rabs (rsub a b)))) (mulSubDistrib R c a b) (replace (\\z => rle z (rmul (radd (rabs c) (rone R)) (rabs (rsub a b)))) (sym (CompleteOrderedField.absMul (field R) c (rsub a b))) (mulLeRight R (rabs c) (radd (rabs c) (rone R)) (rabs (rsub a b)) (absLeAbsPlusOne R c) (CompleteOrderedField.absNonneg (field R) (rsub a b))))
...
limitScalarAll : (R : Real) -> (c : Carrier R) -> (h : Carrier R -> Carrier R) -> (x0 L : Carrier R) -> Limit R h x0 L -> Limit R (\\x => rmul c (h x)) x0 (rmul c L)
limitScalarAll R c h x0 L limH := by
  constructor
  intros eps heps
  constructor
  · exact (DPair.fst (Limit.eps_delta limH (rmul eps (rinv (radd (rabs c) (rone R)))) (epsOverMPos R eps (radd (rabs c) (rone R)) heps (absPlusOnePos R c))))
  · constructor
    · exact (Pair.fst (DPair.snd (Limit.eps_delta limH (rmul eps (rinv (radd (rabs c) (rone R)))) (epsOverMPos R eps (radd (rabs c) (rone R)) heps (absPlusOnePos R c)))))
    · exact (\\x hx0 hxd => replace (\\z => rlt (rabs (rsub (rmul c (h x)) (rmul c L))) z) (mulInvCancel R (radd (rabs c) (rone R)) eps (absPlusOneNe R c)) (leLtTrans R (rabs (rsub (rmul c (h x)) (rmul c L))) (rmul (radd (rabs c) (rone R)) (rabs (rsub (h x) L))) (rmul (radd (rabs c) (rone R)) (rmul eps (rinv (radd (rabs c) (rone R))))) (scalarAbsBound R c (h x) L) (mulLtLeft R (radd (rabs c) (rone R)) (rabs (rsub (h x) L)) (rmul eps (rinv (radd (rabs c) (rone R)))) (absPlusOnePos R c) (Pair.snd (DPair.snd (Limit.eps_delta limH (rmul eps (rinv (radd (rabs c) (rone R)))) (epsOverMPos R eps (radd (rabs c) (rone R)) heps (absPlusOnePos R c)))) x hx0 hxd))))

-- limitScalar: special case for c /= 0 (calls limitScalarAll)
limitScalar : (R : Real) -> (c : Carrier R) -> (Equal c (rzero R) -> Void) -> (f : Carrier R -> Carrier R) -> (x0 L : Carrier R) -> Limit R f x0 L -> Limit R (\\x => rmul c (f x)) x0 (rmul c L)
limitScalar R c hcnz f x0 L limF = limitScalarAll R c f x0 L limF

-- THE DERIVATIVE THEOREM: (c*f)' = c*f'
derivScalar : (R : Real) -> (c : Carrier R) -> (Equal c (rzero R) -> Void) -> (f : Carrier R -> Carrier R) -> (x0 L : Carrier R) -> HasDerivative R f x0 L -> HasDerivative R (\\x => rmul c (f x)) x0 (rmul c L)
derivScalar R c hcnz f x0 L hf = limitExt R (\\x => rmul c (diffQuot R f x0 x)) (diffQuot R (\\y => rmul c (f y)) x0) x0 (rmul c L) (diffQuotScalarEq R c f x0) (limitScalar R c hcnz (diffQuot R f x0) x0 L hf)
...
negZero : (R : Real) -> Equal (rneg (rzero R)) (rzero R) := by
  intros R
  erw (sym (addZeroLeft R (rneg (rzero R)))), (CompleteOrderedField.negRight (field R) (rzero R))
...
subSplit : (R : Real) -> (a b c : Carrier R) -> Equal (radd (rsub a c) (rsub c b)) (rsub a b) := by
  intros R a b c
  erw (CompleteOrderedField.addAssoc (field R) a (rneg c) (rsub c b)), (cong (\\z => radd a z) (trans (sym (CompleteOrderedField.addAssoc (field R) (rneg c) c (rneg b))) (trans (cong (\\z => radd z (rneg b)) (negLeft R c)) (addZeroLeft R (rneg b)))))
...
subTriangle R a b c = replace (\\z => rle (rabs z) (radd (rabs (rsub a c)) (rabs (rsub c b)))) (subSplit R a b c) (CompleteOrderedField.absTriangle (field R) (rsub a c) (rsub c b))
...
mulNegLeft : (R : Real) -> (a b : Carrier R) -> Equal (rmul (rneg a) b) (rneg (rmul a b)) := by
  intros R a b
  erw (CompleteOrderedField.mulComm (field R) (rneg a) b), (mulNegRight R b a), (CompleteOrderedField.mulComm (field R) b a)
...
mulSubDistribRight : (R : Real) -> (a b c : Carrier R) -> Equal (rmul (rsub a b) c) (rsub (rmul a c) (rmul b c)) := by
  intros R a b c
  erw (CompleteOrderedField.distribRight (field R) a (rneg b) c), (mulNegLeft R b c)
...
diffQuotSubMulEq : (R : Real) -> (a b c : Carrier R) -> (Equal b (rzero R) -> Void) -> Equal (rmul (rsub (rmul a (rinv b)) c) b) (rsub a (rmul c b)) := by
  intros R a b c hne
  erw (mulSubDistribRight R (rmul a (rinv b)) c b), (cong (\\z => rsub z (rmul c b)) (trans (CompleteOrderedField.mulAssoc (field R) a (rinv b) b) (trans (cong (\\z => rmul a z) (trans (CompleteOrderedField.mulComm (field R) (rinv b) b) (CompleteOrderedField.mulInvRight (field R) b hne))) (CompleteOrderedField.mulOneRight (field R) a))))
...
derivBoundZero R g y0 Lg eta = replace (\\z => rle z (rmul eta (rabs (rzero R)))) (sym (trans (cong (\\z => rabs (rsub (rsub (g y0) (g y0)) z)) (mulZeroRight R Lg)) (trans (cong (\\z => rabs z) (subZeroRight R (rsub (g y0) (g y0)))) (trans (cong (\\z => rabs z) (subSelf R (g y0))) (absOfZero R))))) (replace (\\z => rle (rzero R) z) (sym (trans (cong (\\z => rmul eta z) (absOfZero R)) (mulZeroRight R eta))) (CompleteOrderedField.leRefl (field R) (rzero R)))
...
derivBoundNonzero : (R : Real) -> (g : Carrier R -> Carrier R) -> (y0 Lg eta : Carrier R) -> (y : Carrier R) -> (Equal (rsub y y0) (rzero R) -> Void) -> rle (rabs (rsub (diffQuot R g y0 y) Lg)) eta -> rle (rabs (rsub (rsub (g y) (g y0)) (rmul Lg (rsub y y0)))) (rmul eta (rabs (rsub y y0)))
derivBoundNonzero R g y0 Lg eta y hne hle = replace (\\z => rle (rabs z) (rmul eta (rabs (rsub y y0)))) (diffQuotSubMulEq R (rsub (g y) (g y0)) (rsub y y0) Lg hne) (replace (\\z => rle z (rmul eta (rabs (rsub y y0)))) (sym (absOfMul R (rsub (diffQuot R g y0 y) Lg) (rsub y y0))) (mulLeRight R (rabs (rsub (diffQuot R g y0 y) Lg)) eta (rabs (rsub y y0)) hle (CompleteOrderedField.absNonneg (field R) (rsub y y0))))
...
derivBound : (R : Real) -> (g : Carrier R -> Carrier R) -> (y0 Lg : Carrier R) -> HasDerivative R g y0 Lg -> (eta : Carrier R) -> rlt (rzero R) eta -> Sigma (Carrier R) (DerivBoundWitness R g y0 Lg eta)
derivBound R g y0 Lg hg eta heta := by
  ...
  · constructor
    ...
    · exact (\\y hyd => eitherElim
        (\\heq => replace (\\z => rle (rabs (rsub (rsub (g z) (g y0)) (rmul Lg (rsub z y0)))) (rmul eta (rabs (rsub z y0)))) (sym (subEqZeroToEq R y y0 heq)) (replace (\\z => rle (rabs (rsub (rsub (g y0) (g y0)) (rmul Lg z))) (rmul eta (rabs z))) (sym (subSelf R y0)) (derivBoundZero R g y0 Lg eta)))
        (\\hne => derivBoundNonzero R g y0 Lg eta y hne (ltToLe R (rabs (rsub (diffQuot R g y0 y) Lg)) eta (Pair.snd (DPair.snd (Limit.eps_delta hg eta heta)) y (absPos R (rsub y y0) hne) hyd)))
        (eqOrNeZero R (rsub y y0)))

-- a < b => a + c < b + c
addLtRight : (R : Real) -> (a b c : Carrier R) -> rlt a b -> rlt (radd a c) (radd b c)
...
absSubAdd R a b = replace (\\z => rle (rabs z) (radd (rabs (rsub a b)) (rabs b))) (subCancel R a b) (CompleteOrderedField.absTriangle (field R) (rsub a b) b)
...
DiffQuotBoundWitness R f x0 Lf df = Pair (rlt (rzero R) df) ((x : Carrier R) -> rlt (rzero R) (rabs (rsub x x0)) -> rlt (rabs (rsub x x0)) df -> rlt (rabs (diffQuot R f x0 x)) (radd (rabs Lf) (rone R)))
...
diffQuotBounded R f x0 Lf hf := by
  ...
  · constructor
    ...
    · exact (\\x hx0 hxd => leLtTrans R (rabs (diffQuot R f x0 x))
        (radd (rabs (rsub (diffQuot R f x0 x) Lf)) (rabs Lf))
        (radd (rabs Lf) (rone R)) (absSubAdd R (diffQuot R f x0 x) Lf)
        (replace (\\z => rlt (radd (rabs (rsub (diffQuot R f x0 x) Lf)) (rabs Lf)) z)
          (CompleteOrderedField.addComm (field R) (rone R) (rabs Lf))
          (addLtRight R (rabs (rsub (diffQuot R f x0 x) Lf)) (rone R) (rabs Lf)
...
diffQuotTimesH : (R : Real) -> (f : Carrier R -> Carrier R) -> (x0 x : Carrier R) -> (Equal (rsub x x0) (rzero R) -> Void) -> Equal (rmul (diffQuot R f x0 x) (rsub x x0)) (rsub (f x) (f x0)) := by
  intros R f x0 x hne
...
-- |diffQuot(f,x0,x)| * |x-x0| = |f(x)-f(x0)| when x-x0 /= 0
absDiffQuotTimesH : (R : Real) -> (f : Carrier R -> Carrier R) -> (x0 x : Carrier R) -> (Equal (rsub x x0) (rzero R) -> Void) -> Equal (rmul (rabs (diffQuot R f x0 x)) (rabs (rsub x x0))) (rabs (rsub (f x) (f x0)))
absDiffQuotTimesH R f x0 x hne = trans (sym (absOfMul R (diffQuot R f x0 x) (rsub x x0))) (cong (\\z => rabs z) (diffQuotTimesH R f x0 x hne))
...
absMulBound R a b M eps hle hlt hM = leLtTrans R (rmul (rabs a) (rabs b)) (rmul M (rabs b)) (rmul M eps) (mulLeRight R (rabs a) M (rabs b) hle (CompleteOrderedField.absNonneg (field R) b)) (mulLtLeft R M (rabs b) eps hM hlt)
...
continuousFromDeriv : (R : Real) -> (f : Carrier R -> Carrier R) -> (x0 Lf : Carrier R) -> HasDerivative R f x0 Lf -> (target : Carrier R) -> rlt (rzero R) target -> Sigma (Carrier R) (ContinuousWitness R f x0 target)
continuousFromDeriv R f x0 Lf hf target htarget := by
  ...
  | Left hle =>
    exact (mkSigma (Carrier R) (ContinuousWitness R f x0 target) (DPair.fst dqb) (MkPair (Pair.fst (DPair.snd dqb)) (\\x hx0 hxd => replace (\\z => rlt z target) (absDiffQuotTimesH R f x0 x (eqOrNeZeroRight R (rsub x x0) (Pair.snd hx0))) (replace (\\z => rlt (rmul (rabs (diffQuot R f x0 x)) (rabs (rsub x x0))) z) (mulInvCancel R (radd (rabs Lf) (rone R)) target (absPlusOneNe R Lf)) (absMulBound R (diffQuot R f x0 x) (rsub x x0) (radd (rabs Lf) (rone R)) (rmul target (rinv (radd (rabs Lf) (rone R)))) (ltToLe R (rabs (diffQuot R f x0 x)) (radd (rabs Lf) (rone R)) (Pair.snd (DPair.snd dqb) x hx0 hxd)) (ltLeTrans R (rabs (rsub x x0)) (DPair.fst dqb) (rmul target (rinv (radd (rabs Lf) (rone R)))) hxd hle) (absPlusOnePos R Lf))))))
  | Right hle =>
    exact (mkSigma (Carrier R) (ContinuousWitness R f x0 target) (rmul target (rinv (radd (rabs Lf) (rone R)))) (MkPair (epsOverMPos R target (radd (rabs Lf) (rone R)) htarget (absPlusOnePos R Lf)) (\\x hx0 hxd => replace (\\z => rlt z target) (absDiffQuotTimesH R f x0 x (eqOrNeZeroRight R (rsub x x0) (Pair.snd hx0))) (replace (\\z => rlt (rmul (rabs (diffQuot R f x0 x)) (rabs (rsub x x0))) z) (mulInvCancel R (radd (rabs Lf) (rone R)) target (absPlusOneNe R Lf)) (absMulBound R (diffQuot R f x0 x) (rsub x x0) (radd (rabs Lf) (rone R)) (rmul target (rinv (radd (rabs Lf) (rone R)))) (Pair.snd (DPair.snd dqb) x hx0 (ltLeTrans R (rabs (rsub x x0)) (rmul target (rinv (radd (rabs Lf) (rone R)))) (DPair.fst dqb) hxd hle)) hxd (absPlusOnePos R Lf))))))
...
chainAlgId : (R : Real) -> (g f : Carrier R -> Carrier R) -> (x0 Lg : Carrier R) -> (x : Carrier R) -> Equal (radd (chainTermA R g f x0 Lg x) (rmul Lg (diffQuot R f x0 x))) (diffQuot R (\\y => g (f y)) x0 x) := by
  intros R g f x0 Lg x
  erw (cong (\\z => radd (chainTermA R g f x0 Lg x) z) (sym (CompleteOrderedField.mulAssoc (field R) Lg (rsub (f x) (f x0)) (rinv (rsub x x0))))), (sym (CompleteOrderedField.distribRight (field R) (rsub (rsub (g (f x)) (g (f x0))) (rmul Lg (rsub (f x) (f x0)))) (rmul Lg (rsub (f x) (f x0))) (rinv (rsub x x0)))), (cong (\\z => rmul z (rinv (rsub x x0))) (subCancel R (rsub (g (f x)) (g (f x0))) (rmul Lg (rsub (f x) (f x0)))))
...
mulAssocAbs : (R : Real) -> (eta a b : Carrier R) -> Equal (rmul (rmul eta (rabs a)) (rabs b)) (rmul eta (rabs (rmul a b))) := by
  intros R eta a b
  erw (CompleteOrderedField.mulAssoc (field R) eta (rabs a) (rabs b)), (cong (\\z => rmul eta z) (sym (absOfMul R a b)))
...
chainBound R num fxfx0 h eta M eps hdb hdq heta hmul = replace (\\z => rlt (rabs (rmul num (rinv h))) z) hmul (leLtTrans R (rabs (rmul num (rinv h))) (rmul eta (rabs (rmul fxfx0 (rinv h)))) (rmul eta M) (replace (\\z => rle z (rmul eta (rabs (rmul fxfx0 (rinv h))))) (sym (absOfMul R num (rinv h))) (replace (\\z => rle (rmul (rabs num) (rabs (rinv h))) z) (mulAssocAbs R eta fxfx0 (rinv h)) (mulLeRight R (rabs num) (rmul eta (rabs fxfx0)) (rabs (rinv h)) hdb (CompleteOrderedField.absNonneg (field R) (rinv h))))) (mulLtLeft R eta (rabs (rmul fxfx0 (rinv h))) M heta hdq))
...
chainTermALimit R g f x0 Lg Lf hf hg := by
  ...
  have hepsM := epsOverMPos R eps (radd (rabs Lf) (rone R)) heps (absPlusOnePos R Lf)
  ...
  | Left hle =>
    exact (MkDPair (DPair.fst cfd) (MkPair (Pair.fst (DPair.snd cfd)) (\\x hx0 hxd => replace (\\z => rlt (rabs z) eps) (sym (subZeroRight R (chainTermA R g f x0 Lg x))) (chainBound R (rsub (rsub (g (f x)) (g (f x0))) (rmul Lg (rsub (f x) (f x0)))) (rsub (f x) (f x0)) (rsub x x0) epsM (radd (rabs Lf) (rone R)) eps (Pair.snd (DPair.snd db) (f x) (Pair.snd (DPair.snd cfd) x hx0 hxd)) (Pair.snd (DPair.snd dqb) x hx0 (ltLeTrans R (rabs (rsub x x0)) (DPair.fst cfd) (DPair.fst dqb) hxd hle)) hepsM (trans (CompleteOrderedField.mulComm (field R) epsM (radd (rabs Lf) (rone R))) (mulInvCancel R (radd (rabs Lf) (rone R)) eps (absPlusOneNe R Lf)))))))
  | Right hle =>
    exact (MkDPair (DPair.fst dqb) (MkPair (Pair.fst (DPair.snd dqb)) (\\x hx0 hxd => replace (\\z => rlt (rabs z) eps) (sym (subZeroRight R (chainTermA R g f x0 Lg x))) (chainBound R (rsub (rsub (g (f x)) (g (f x0))) (rmul Lg (rsub (f x) (f x0)))) (rsub (f x) (f x0)) (rsub x x0) epsM (radd (rabs Lf) (rone R)) eps (Pair.snd (DPair.snd db) (f x) (Pair.snd (DPair.snd cfd) x hx0 (ltLeTrans R (rabs (rsub x x0)) (DPair.fst dqb) (DPair.fst cfd) hxd hle))) (Pair.snd (DPair.snd dqb) x hx0 hxd) hepsM (trans (CompleteOrderedField.mulComm (field R) epsM (radd (rabs Lf) (rone R))) (mulInvCancel R (radd (rabs Lf) (rone R)) eps (absPlusOneNe R Lf)))))))
...
--                      = g' * f'                              [addZeroLeft]
derivChain : (R : Real) -> (g f : Carrier R -> Carrier R) -> (x0 Lf Lg : Carrier R) -> HasDerivative R f x0 Lf -> HasDerivative R g (f x0) Lg -> HasDerivative R (\\x => g (f x)) x0 (rmul Lg Lf)
derivChain R g f x0 Lf Lg hf hg := by
  -- Reduce to proving the limit of A(x) + Lg*diffQuot(f, x0, x)
  suffices h : Limit R (\\x => radd (chainTermA R g f x0 Lg x) (rmul Lg (diffQuot R f x0 x))) x0 (rmul Lg Lf) by
    exact limitExt R (\\x => radd (chainTermA R g f x0 Lg x) (rmul Lg (diffQuot R f x0 x))) (diffQuot R (\\y => g (f y)) x0) x0 (rmul Lg Lf) (chainAlgId R g f x0 Lg) h
  ...
  have hSum := limitAdd R (chainTermA R g f x0 Lg) (\\x => rmul Lg (diffQuot R f x0 x)) x0 (rzero R) (rmul Lg Lf) hA hScale
  -- Rewrite 0 + Lg*Lf = Lg*Lf
  exact replace (\\z => Limit R (\\x => radd (chainTermA R g f x0 Lg x) (rmul Lg (diffQuot R f x0 x))) x0 z) (addZeroLeft R (rmul Lg Lf)) hSum
`;
