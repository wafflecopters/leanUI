...
plus (Succ a) b = Succ (plus a b)`;
  ...
  assert(plusDecl.surfaceValue?.tag === 'Match', 'plus should have Match value');
  if (plusDecl.surfaceValue?.tag === 'Match') {
    console.log(`  plus has ${plusDecl.surfaceValue.clauses.length} pattern clauses`);
    assert(plusDecl.surfaceValue.clauses.length === 2, 'plus should have 2 clauses');
  }
});
